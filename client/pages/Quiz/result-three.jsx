import React from 'react';

export default class Result3 extends React.Component {
  componentDidMount() {
    window.scrollTo(0, 0);
  }

  render() {
    return (
      <div>
        <div className="container-fluid purple-background">
          <div className="justify-content-center text-center padding-top3 padding-bottom3">
            <span className="cormorant-garamond-light">
              &quot;Just because no one else can heal or do your inner work for
              you <br></br>doesn&apos;t mean you can, should, or need to do it
              alone.&quot;<br></br> -Lisa Olivera
            </span>
          </div>
        </div>
        <div className="container-fluid grey-background">
          <div className="justify-content-center text-center padding-top3">
            <i className="fas fa-star-and-crescent padding-bottom3"></i>
          </div>
          <div className="col-8 mx-auto padding-bottom3">
            <img src="https://www.nami.org/NAMI/media/NAMI-Media/BlogImageArchive/2019/kids-with-mental-illness-blog.jpeg"></img>
            <div className="justify-content-center text-center">
              <p className="cormorant-garamond-light padding-top">
                Good job in taking your first step to a more natural way to
                better your mental health. <br></br>Here are a few suggestions
                on what you can do to include in your everyday routine:
              </p>
              <div>
                <i className="fas fa-heart padding-top5 padding-bottom5"></i>
              </div>
              <div>
                <i className="fas fa-arrow-down padding-top5 padding-bottom"></i>
              </div>
            </div>
          </div>
          <div className="row justify-content-center text-center padding-top3 padding-bottom">
            <div className="col-6 mx-auto text-center">
              <p className="cormorant-garamond-bold padding-top2 padding-bottom">
                your score:
              </p>
              <span className="roboto-text">
                You&apos;re unhappier than average. Okay, so you have some work
                to do, but the good news is you can easily shift your mood with
                a little practice. Start by accepting what you&apos;re feeling
                rather than invalidating it and embrace all feelings with
                curiosity and nonjudgement. Be open to the awareness that those
                feelings are only part of the story. It also means you might
                have some old wounds that need healing. When you treat yourself
                with compassion, & live in alignment with your values, you show
                up in a kinder way for others and are better able to experience
                the joy of the moment and see the good in others. That is a
                deeply satisfying place that brings waves of contentment. Focus
                on gratitude, find the good in yourself and others, & create a
                narrative that brings meaning to your life experience. Recognize
                that each of us is a small part of a whole & we are never alone
                with our suffering or our joy. Just by taking this quiz,
                you&apos;re already on your way to leading a happier, more
                meaningful life. Jot something down
              </span>
            </div>
          </div>
          <div className="col-10 mx-auto padding-bottom5">
            <div className="row justify-content-center">
              <div className="text-center">
                <div>
                  <i className="fas fa-arrow-down padding-top5 padding-bottom5"></i>
                </div>
                <div>
                  <i className="fas fa-medkit padding-top2"></i>
                </div>
              </div>
              <p className="cormorant-garamond-bold text-center padding-bottom">
                self-care 101:
              </p>
            </div>
            <div className="row justify-content-center padding-bottom-sm">
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-seedling"></i>
                </div>
              </div>
              <div className="col-2">
                <div className="row roboto-heading">Take care of basics.</div>
                <div className="row cormorant-garamond-light-subtext">
                  Try to eat well and get enough sleep. Sleep & nutrition are
                  very important.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-grip-lines-vertical"></i>
                </div>
              </div>
              <div className="col-2">
                <div className="row roboto-heading">Be kind to yourself.</div>
                <div className="row cormorant-garamond-light-subtext">
                  Set realistic expectations & be patient with yourself &
                  others.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-grip-lines-vertical"></i>
                </div>
              </div>
              <div className="col-2">
                <div className="row roboto-heading">Give & accept support.</div>
                <div className="row cormorant-garamond-light-subtext">
                  Expect to feel angry, sad, confused or just numb - & talk to
                  others about it.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-phone"></i>
                </div>
              </div>
            </div>
            <hr></hr>
            <div className="row justify-content-center padding-bottom-sm padding-top-sm">
              <div className="col-3">
                <div className="row roboto-heading">
                  Get back to your daily routine.
                </div>
                <div className="row cormorant-garamond-light-subtext">
                  Doing things as simple as brushing your teeth can help restore
                  a sense of safety & control.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-grip-lines-vertical"></i>
                </div>
              </div>
              <div className="col-5">
                <div className="row roboto-heading">Take a break.</div>
                <div className="row cormorant-garamond-light-subtext">
                  Take breaks from disaster news coverage & from thinking &
                  talking about the events. Ignore your phone for a few minutes,
                  clear your head & go for a walk.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-mug-hot"></i>
                </div>
              </div>
            </div>
            <hr></hr>
            <div className="row justify-content-center padding-bottom-sm padding-top">
              <div className="col-1">
                <div className="text-center padding-top">
                  <i className="far fa-thumbs-up"></i>
                </div>
              </div>
              <div className="col-2">
                <div className="row roboto-heading">Say yes & no.</div>
                <div className="row cormorant-garamond-light-subtext">
                  People who are open to new experiences see the world
                  differently according to research. But, setting boundaries on
                  your time is also essential.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-grip-lines-vertical"></i>
                </div>
              </div>
              <div className="col-2">
                <div className="row roboto-heading">Make time for friends.</div>
                <div className="row cormorant-garamond-light-subtext">
                  Close relationships have a wide range of benefits: they
                  motivate us, inform us, help us grow. And, they also have
                  substantial mental health benefits.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center">
                  <i className="padding-top fas fa-grip-lines-vertical"></i>
                </div>
              </div>
              <div className="col-2">
                <div className="row roboto-heading">Turn off social media.</div>
                <div className="row cormorant-garamond-light-subtext">
                  It&apos;s the nutrition equivalent of eating a big bag of
                  chips: satisfying in the moment, addicting, & leaves you
                  feeling remorseful & maybe a little sick.
                </div>
              </div>
              <div className="col-1">
                <div className="text-center padding-top">
                  <i className="fas fa-comment-slash"></i>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}
